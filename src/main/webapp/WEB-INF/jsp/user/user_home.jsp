...
<link href="<c:url value="/template/user_tpl/css/ui.css"/>" rel="stylesheet">
								...
								<div class="product-image-wrapper">
									<div class="single-products">
										<div class="productinfo text-center">
											...
											<a href="/detail/${product.uniquename}" class="my-buton btn btn-default add-to-cart"><i class="fa fa-shopping-cart"></i>Szczegóły</a>
										</div>
					...
					<div class="paging-div">
						<c:if test="${totalPages > 1}">
							...
							<c:if test="${currentPage > 1}">
								<spring:url value="/page/1" var="firstP" />
							...
							<c:if test="${currentPage > 1}">
								<spring:url value="/page/${currentPage - 1}" var="previousP" />
							...
							<c:forEach var="i" begin="1" end="${totalPages}">
								<span> <c:if test="${currentPage != i}">
										<spring:url value="/page/${i}" var="iPage" />
										<a href="${iPage}">${i}</a>
							...
							<c:if test="${currentPage < totalPages}">
								<spring:url value="/page/${currentPage + 1}" var="nextP" />
								<a href="${nextP}">Next</a>
								<c:if test="${currentPage >= totalPages}">
									<span>Next</span>
								</c:if>
							...
							&nbsp;&nbsp;
							<c:if test="${currentPage < totalPages}">
								<spring:url value="/page/${totalPages}" var="lastP" />
								<a href="${lastP}">Last</a>
							</c:if>
							<c:if test="${currentPage >= totalPages}">
								<span>Last</span>
